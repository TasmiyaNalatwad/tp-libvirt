...
#
# Author: Tasmiya.Nalatwad<tasmiya@linux.ibm.com>
...
def run(test, params, env):
    ...
    def get_interface_list_before_hotplug(session):
        """
        ...
        interface_list_before = output.splitlines()
        logging.debug("interface_list before hotplug %s", interface_list_before)

        return interface_list_before
    ...
    interface_list_before = get_interface_list_before_hotplug(session)
    obj = PciAssignable()
    ...
    def test_ping(net_ip, server_ip, netmask):
        ...
        try:
            ...
            if sorted(vm.get_pci_devices()) != sorted(nic_list_before):
                logging.debug("The adapter has been successfully attached to the guest")
            else:
            ...
            for val in range(len(nic_list)):
                pci_device_address.append(str(nic_list[val]).split(' ', 1)[0])
                nic_devices.append(str(nic_list[val]).split(' ', 1)[0][:-2])
            pci_device_address.sort()
            if not sriov:
    ...
    def check_attach_pci():
        ...
        :return : Returns True if the pci device interface is found in the guest interface list
                : Return False if the pci device interface is not found in the guest interface list
        """
        session = vm.wait_for_login()
        output = session.cmd_output("ip link")
        interface_list_after = str(output.splitlines())
        logging.debug(interface_list_after)
        return interface_list_after != interface_list_before
    ...
    def test_eeh_nic():
            ...
            if num_of_miss < 5:
                ...
                if not is_hit:
                    ...
                    if num_of_hit >= 1 and pass_hit != 0:
                        test.fail(f"Failed to inject EEH after {pass_hit} successful attempt for {pci_ids}. Please check dmesg logs")
        ...
        if is_removed:
            ...
            test.fail(f"PCI Device {pci_ids} failed to permanently disable after max hit")
